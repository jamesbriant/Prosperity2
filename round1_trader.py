...
class Trader:    
    ...
    # def calc_acceptable_price_starfruit_grad(
    #     self,
    #     price_history: list[float],
    # ) -> int:
    #     n = len(price_history)
    ...
    #     # Calculate the average price of the last 6 prices
    #     avg6_grad = sum(self.subtract_lists(price_history[-6:], price_history[-7:-1])) / 6

    #     # Calculate the average price of the last n prices
    #     avg_grad = sum(self.subtract_lists(price_history[1:], price_history[:-1])) / (n-1)

    #     grad = 0.5 * avg_grad + 0.5 * avg6_grad

    #     return int(round(price_history[-1] + grad))
